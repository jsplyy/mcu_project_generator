...
/*
 * rotl() and next() functions in this file are taken from
 ...
 */
#define FSL_LTC_DPA_HAVE_GATE_OFF 0
...
#define FSL_LTC_DPA_ACT_ND_AES 19u
...
struct _ltc_dpa_req_handle
{
    ...
    uint32_t rngContext[4];       /*!< Internal Golden ration PRNG state. Use for permutations. */
    uint64_t prngState[2];        /*!< Internal PRNG state. Use for random key bits. */
    union
    {
        uint64_t prngData; /*!< Random bits. */
        uint32_t prngDataWord[2];
    };
    uint32_t prngCount; /*!< Number of bits available. */
...
static uint64_t next(ltc_dpa_req_handle_t *reqHandle)
{
    const uint64_t s0 = reqHandle->prngState[0];
    uint64_t s1 = reqHandle->prngState[1];
    const uint64_t result = s0 + s1;

    s1 ^= s0;
    reqHandle->prngState[0] = rotl(s0, 55) ^ s1 ^ (s1 << 14); /* a, b */
    reqHandle->prngState[1] = rotl(s1, 36);                   /* c */

    return result;
}

static uint32_t ltc_prng_get_random_key32(ltc_dpa_req_handle_t *reqHandle)
{
    uint32_t retVal;
    if (0 == reqHandle->prngCount)
    {
        reqHandle->prngData = next(reqHandle);
        reqHandle->prngCount = 2u;
    }

    retVal = reqHandle->prngDataWord[2 - reqHandle->prngCount];
    reqHandle->prngCount--;
...
{
    uint32_t r32 = getRandomWord(reqHandle->rngContext);
    reqHandle->gateOffRandom = r32;
    reqHandle->gateOffRepeat = 1u + ((r32 & 0x0000000Fu));
    reqHandle->gateOffCycles = ((r32 & 0x000000F0u) >> 4);
    ...
    if (reqHandle->gateOffCycles > reqHandle->gateOffCyclesMax)
    {
        reqHandle->gateOffCycles = reqHandle->gateOffCyclesMax;
    }
}
...
static status_t ltc_one_block(LTC_Type *base, ltc_dpa_req_handle_t *reqHandle, uint8_t *output, const uint8_t *input)
{
    ...
    /* always start with no random key bits, regardless prior processing (e.g.SetKeyDPA) */
    reqHandle->prngCount = 0;
...
status_t LTC_CreateHandleDPA(LTC_Type *base, ltc_dpa_handle_t *handle, uint8_t seed[16])
{
    ...
    ltc_memcpy(reqHandle->prngState, seed, 16);
...
#if defined(FSL_LTC_DPA_HAVE_GATE_OFF) && (FSL_LTC_DPA_HAVE_GATE_OFF > 0)
    LTC_SetGateOffRepeatDPA(handle, 16u);
